...
async function find(req: Request, res: Response)  {
    try {
        ...
        const {
            ...
            title = '', search_after = '', size = 0, 
            sort_by_fields = '', get_fields = ''
        } = req.query;
        ...
        const sort_list = sort_fields.map((type: string) => {
        ...
        const res_data = hits.map((item: any) => {
            const { _source = {}} = item;
